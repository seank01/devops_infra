...
def alb_read_and_convert(number, line, outputformat):
  ...
  fields = re.compile(
    r'([^ ]*) ([^ ]*) ([^ ]*) ([^ ]*):([0-9]*) ([^ ]*)[:-]([0-9]*) ([-.0-9]*) ([-.0-9]*) ([-.0-9]*) (|[-0-9]*) (-|[-0-9]*) ([-0-9]*) ([-0-9]*) \"([^ ]*) ([^ ]*) (- |[^ ]*)\" \"([^\"]*)\" ([A-Z0-9-]+) ([A-Za-z0-9.-]*) ([^ ]*) \"([^\"]*)\" \"([^\"]*)\" \"([^\"]*)\" ([-.0-9]*) ([^ ]*) \"([^\"]*)\" \"([^\"]*)\"($| \"[^ ]*\")(.*)').findall(
    line)[0]
  timestamp = 1000 * (datetime.datetime.strptime(fields[1],
                                                 '%Y-%m-%dT%H:%M:%S.%fZ').timestamp())  # todo : find a more efficient way to convert date/time in log file (string) to a timestamp (int)
...
def lambda_handler(event, context):
  ...
  loggroup = os.environ.get('CW_LOG_GROUP', '')
  inputformat = os.environ.get('INPUT_FORMAT', '')
  ...
  if (inputformat == '' or outputformat == ''):
    logging.warning(
      "Env var InputFormat or OutputFormat not defined, log stream will not be converted before sending to CloudWatch Logs")
  ...
  for line in f:
    ...
    if (timestamp == 0 or message == None):
      ...
      if (i == 1):
        ...
        print(line)
      if (
              i % 500 == 0):  # a higher value may result of a buffer overflow from Boto3 because you cant send more than 1MB of events in one call
